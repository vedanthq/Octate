...
Biome replaces a separate ESLint + Prettier setup with a single fast tool.

Use Jest for unit and integration tests.

Do not introduce additional linting, formatting, or test tooling unless a real gap appears.
...
TypeScript compiler
ESLint or Biome where configured
...
The exclusion of a GitLab App in §3 and §92 refers specifically to a heavy, webhook-based GitLab integration — the same thing being excluded for GitHub. It does not rule out a thin GitLab CI adapter later, following the same Phase A pattern above: a CI job that runs `octate review --json` or `--sarif` and posts results, with no hosted service required.
...
The terminal is the product surface. Build the review engine underneath it before building anything around it.

127. Competitive and Strategic Context — Cubic and Jules
...
↓
Context Assembly (Codegraph + team history + business context)
↓
